...
import { fetchOcf3BySlug } from "../helpers/fetchOcf3BySlug";
import { createOcf3 } from "../helpers/createOcf3";
...
export default function Ocf3Page() {
  ...
  const { data: ocf3Response, isLoading: loadingOcf3 } = useQuery({
    queryKey: ["ocf3", slug],
    queryFn: () => fetchOcf3BySlug(slug),
    enabled: !!slug,
  });

  // Extract data from response
  const ocf3Data = ocf3Response?.response || ocf3Response;

  const saveMutation = useMutation({
    mutationFn: createOcf3,
    onSuccess: () => {
      toast.success("OCF-3 data saved successfully!");
    },
    onError: () => {
      toast.error("Failed to save OCF-3 data");
    },
  });
  ...
  useEffect(() => {
    if (ocf3Data) {
      setFormData({
        health_practitioer: ocf3Data.health_practitioer || "",
        completed_by_id: ocf3Data.completed_by_id || "",
        date_of_completion: ocf3Data.date_of_completion || "",
        date: ocf3Data.date || "",
        via_id: ocf3Data.via_id || "",
        date_2: ocf3Data.date_2 || "",
        via_2_id: ocf3Data.via_2_id || "",
      });
    }
  }, [ocf3Data]);
    ...
    <div className="min-h-screen bg-muted">
      ...
      <nav className="bg-card border-b px-6 py-4 text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-foreground font-medium">OCF-3</span>
        </div>
      </nav>

      <main className="container mx-auto px-6 py-8 max-w-7xl">
        <h1 className="text-2xl font-bold mb-6 text-foreground">OCF-3 Form</h1>
        <form
          className="bg-card rounded-lg shadow-sm border p-8 space-y-8"
          onSubmit={handleSubmit}
        >
          {/* Row 1: Health Practitioner, Completed By, Date of Completion */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <DatePicker
              label="Health Practitioner"
              value={formData.health_practitioer}
              onChange={(val) => handleDateChange("health_practitioer", val)}
            />

            <SearchableSelect
              label="Completed By"
              options={meta.tracking_ocf_3_completed_by || []}
          ...
          {/* Row 2: Date (Sent to AB), Via, Date 2 */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <DatePicker
