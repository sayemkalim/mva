...
export const createPsychological = async ({ slug, data }) => {
  try {
    if (!slug) throw new Error("Slug is required for creating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.createPsychological}/save/${slug}`,
      method: "POST",
      data,
      headers: {
...
export const updatePsychological = async (id, data) => {
  try {
    if (!id) throw new Error("ID is required for updating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.updatePsychological}/update/${id}`,
      method: "POST",
      data,
      headers: {
        "Content-Type": "application/json",
