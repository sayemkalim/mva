...
import { fetchOcfProd3List } from "../helpers/fetchOcfProd2List";
import { deleteOcfProd3List } from "../helpers/deleteOcfProd3List";
...
const OcfProdTable = ({ slug, setBlogsLength, params, setParams }) => {
  ...
  const {
    ...
    queryKey: ["ocfProd5List", slug, params],
    queryFn: () => fetchOcfProd3List(slug, params),
    enabled: !!slug,
  });
  ...
  const { mutate: deleteSectionMutation, isLoading: isDeleting } = useMutation({
    mutationFn: (id) => deleteOcfProd3List(id),
    onSuccess: () => {
      toast.success("ocfProdList deleted successfully.");
      queryClient.invalidateQueries(["ocfProd3List", slug]);
      onCloseDialog();
    },
    onError: () => {
  ...
  const { mutate: copyMutation, isLoading: isCopying } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["ocfProd5List", slug]);
      setOpenImport(false);
      setSelectedImport("");
    },
    onError: (error) => {
  ...
  const onNavigateToEdit = (section) => {
    ...
    navigate(`/dashboard/ocf5-prod/edit/${section.id}`);
  };
