...
export const createOcfProd3 = async ({ slug, data }) => {
  try {
    if (!slug) throw new Error("Slug is required for creating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.createOcfProd3}/save/${slug}`,
      method: "POST",
      data,
      headers: {
...
export const updateOcfProd3 = async (id, data) => {
  try {
    if (!id) throw new Error("ID is required for updating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.updateOcfProd5}/update/${id}`,
      method: "POST",
      data,
      headers: {
        "Content-Type": "application/json",
