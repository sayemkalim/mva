...
export const fetchTpCounselBySlug = async (slug) => {
  try {
    console.log("🔍 Fetching counsel with slug:", slug);
    console.log("📍 API Endpoint:", `${endpoints.tpCounsel}/${slug}`);

    const apiResponse = await apiService({
      endpoint: `${endpoints.tpCounsel}/${slug}`,
      method: "GET",
    });
    ...
    // 1) Sabse pehle data / response ko normalize karo
    let payload = null;

    if (apiResponse.data !== undefined) {
      payload = apiResponse.data;
    } else if (apiResponse.response !== undefined) {
      // Yahi case tumhare error ka hai
      // If response is empty string => treat as "no record yet"
      if (
        ...
        return null; // 👉 IMPORTANT: error nahi, simply null
      }
