...
import { Pencil, Printer, Trash2, Check, ChevronsUpDown, Loader2, Import } from "lucide-react";
...
import { fetchSocProdList } from "../helpers/fetchOcfProdList";
import { deleteSocProdList } from "../helpers/deleteSocProdList";
import { printSocProd } from "../helpers/printOcfProd";
...
const SocProdTable = ({ slug, setBlogsLength, params, setParams }) => {
  ...
  const {
    ...
    queryKey: ["socProdList", slug, params],
    queryFn: () => fetchSocProdList(slug, params),
    enabled: !!slug,
  });
  ...
  const { mutate: deleteSectionMutation, isLoading: isDeleting } = useMutation({
    mutationFn: (id) => deleteSocProdList(id),
    onSuccess: () => {
      toast.success("socProdList deleted successfully.");
      queryClient.invalidateQueries(["socProdList", slug]);
      onCloseDialog();
    },
    onError: () => {
  ...
  const onNavigateToEdit = (section) => {
    ...
    navigate(`/dashboard/soc-prod/edit/${section.id}`);
  };
    ...
    <>
      ...
      <Dialog open={openImport} onOpenChange={setOpenImport}>
        <DialogContent className="sm:max-w-[800px] min-h-[500px] flex flex-col">
          <DialogHeader>
            <DialogTitle>Import SOC Data</DialogTitle>
            <DialogDescription>
              ...
              <Popover open={openDropdown} onOpenChange={setOpenDropdown} modal={false}>
                ...
                <PopoverContent className="w-[750px] p-0" align="start">
                  <Command>
                    ...
                    <CommandList
                      className="max-h-[225px] w-full overflow-y-auto overflow-x-hidden"
