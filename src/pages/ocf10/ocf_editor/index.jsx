...
import { fetchOcf10BySlug } from "../helpers/fetchOcf10BySlug";
import { createOcf10 } from "../helpers/createOcf10";
...
export default function Ocf10Page() {
  ...
  const { data: ocf10Response, isLoading: loadingOcf10 } = useQuery({
    queryKey: ["ocf10", slug],
    queryFn: () => fetchOcf10BySlug(slug),
    enabled: !!slug,
  });

  // Extract data from response
  const ocf10Data = ocf10Response?.response || ocf10Response;

  const saveMutation = useMutation({
    mutationFn: createOcf10,
    onSuccess: () => {
      toast.success("OCF-10 data saved successfully!");
      navigate("/dashboard/workstation");
    },
    onError: () => {
      toast.error("Failed to save OCF-10 data");
    },
  });
  ...
  useEffect(() => {
    if (ocf10Data) {
      setFormData({
        completed_by_id: ocf10Data.completed_by_id || "",
        date_of_completion: ocf10Data.date_of_completion || "",
        date: ocf10Data.date || "",
        via_id: ocf10Data.via_id || "",
        date_2: ocf10Data.date_2 || "",
        via_2_id: ocf10Data.via_2_id || "",
      });
    }
  }, [ocf10Data]);
    ...
    <div className="min-h-screen bg-muted">
      ...
      <nav className="bg-card border-b px-6 py-4 text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-foreground font-medium">OCF-10</span>
        </div>
      </nav>

      <main className="container mx-auto px-6 py-8 max-w-7xl">
        <h1 className="text-2xl font-bold mb-6 text-foreground">OCF-10 Form</h1>
        <form
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <SearchableSelect
              label="Completed By"
              options={meta.tracking_ocf_10_completed_by || []}
