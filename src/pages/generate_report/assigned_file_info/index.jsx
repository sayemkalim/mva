...
import {
  exportFileAssignedInfo,
  getFileAssignedMetadata,
} from "./helpers/exportFileAssignedInfo";
...
import { getABMeta } from "@/pages/task/helpers/fetchABMeta";

const ExportFileAssignedInfo = () => {
  ...
  const { data: metadata, isLoading: isLoadingMetadata } = useQuery({
    ...
    queryFn: async () => {
      const response = await getFileAssignedMetadata();
      return response?.response || response;
    },
  });

  const { data: taskMeta, isLoading: isLoadingTaskMeta } = useQuery({
    queryKey: ["taskMeta"],
    queryFn: async () => {
      const response = await getABMeta();
      return response?.response || response;
    },
  });
    ...
    try {
      ...
      switch (formData.type) {
        case "select_file":
          ...
          break;
        case "assigned_by":
          filters.assigned_by = formData.assigned_by;
          break;
        case "Year":
    ...
    <div className="flex flex-col h-screen bg-white">
      ...
      <nav className="bg-white border-b px-6 py-4 text-sm text-gray-600">
        <div className="flex items-center gap-2">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-gray-900 font-medium">File Assigned Info</span>
        </div>
      </nav>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent side="bottom" avoidCollisions={false}>
                      ...
                      <SelectItem value="select_file">Select file</SelectItem>
                      <SelectItem value="assigned_by">Assigned by</SelectItem>
                ...
                {/* Select File - ContactSearch */}
                {formData.type === "select_file" && (
                  <ContactSearch
                ...
                {/* Assigned By - ContactSearch */}
                {formData.type === "assigned_by" && (
                  <div className="space-y-2">
              ...
              {/* Right Column - Select Applicant for relevant types */}
              <div className="space-y-6">
                {formData.type === "select_file" ||
                  formData.type === "assigned_by"}
              </div>
...
export default ExportFileAssignedInfo;
