...
import { fetchOcfProd6List } from "../helpers/fetchOcfProd6List";
import { deleteOcfProd6List } from "../helpers/deleteOcfProd6List";
...
const OcfProdTable = ({ slug, setBlogsLength }) => {
  ...
  const {
    ...
    queryKey: ["ocfProd6List", slug],
    queryFn: () => fetchOcfProd6List(slug),
    enabled: !!slug,
  });
  ...
  const { mutate: deleteSectionMutation, isLoading: isDeleting } = useMutation({
    mutationFn: (id) => deleteOcfProd6List(id),
    onSuccess: () => {
      toast.success("ocfProdList deleted successfully.");
      queryClient.invalidateQueries(["ocfProd6List", slug]);
      onCloseDialog();
    },
    onError: () => {
  ...
  const onNavigateToEdit = (section) => {
    ...
    navigate(`/dashboard/ocf6-prod/edit/${section.id}`);
  };
