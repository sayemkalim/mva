...
export const createOcfProd10 = async ({ slug, data }) => {
  try {
    if (!slug) throw new Error("Slug is required for creating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.createOcfProd10}/save/${slug}`,
      method: "POST",
      data,
      headers: {
...
export const updateOcfProd10 = async (id, data) => {
  try {
    if (!id) throw new Error("ID is required for updating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.updateOcfProd10}/update/${id}`,
      method: "POST",
      data,
      headers: {
        "Content-Type": "application/json",
