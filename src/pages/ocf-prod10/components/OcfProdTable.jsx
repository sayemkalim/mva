...
import { deleteOcfProd10List } from "../helpers/deleteOcfProd10List";
import { printOcfProd10 } from "../helpers/printOcfProd";
import { fetchOcfProd10List } from "../helpers/fetchOcfProd10List";
...
const OcfProdTable = ({ slug, setBlogsLength, params, setParams }) => {
  ...
  const {
    ...
    queryKey: ["ocfProd10List", slug, params],
    queryFn: () => fetchOcfProd10List(slug, params),
    enabled: !!slug,
  });
  ...
  const { mutate: deleteSectionMutation, isLoading: isDeleting } = useMutation({
    mutationFn: (id) => deleteOcfProd10List(id),
    onSuccess: () => {
      toast.success("ocfProdList deleted successfully.");
      queryClient.invalidateQueries(["ocfProd10List", slug]);
      onCloseDialog();
    },
    onError: () => {
  ...
  const { mutate: printMutation, isLoading: isPrinting } = useMutation({
    mutationFn: (id) => printOcfProd10(id),
    onSuccess: () => {
  ...
  const onNavigateToEdit = (section) => {
    ...
    navigate(`/dashboard/ocf10-prod/edit/${section.id}`);
  };
