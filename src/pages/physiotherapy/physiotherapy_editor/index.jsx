...
import { fetchPhysiotherapyBySlug } from "../helpers/fetchPhysiotherapyBySlug";
import { createPhysiotherapy } from "../helpers/createPhysiotherapy";
import { deletePhysiotherapy } from "../helpers/deletePhysiotherapy";
...
export default function PhysiotherapyPage() {
  ...
  const { data: physiotherapyResponse, isLoading: loadingPhysiotherapy } =
    useQuery({
      queryKey: ["physiotherapy", slug],
      queryFn: () => fetchPhysiotherapyBySlug(slug),
      enabled: !!slug,
    });

  const physiotherapyData =
    physiotherapyResponse?.response || physiotherapyResponse || [];

  const saveMutation = useMutation({
    mutationFn: createPhysiotherapy,
    onSuccess: (data) => {
      ...
      toast.success(resp?.message || "Physiotherapy data saved successfully!");
    ...
    onError: (error) => {
      ...
      if (errorData?.Apistatus === false) {
        ...
        toast.error(errorData?.message || "Failed to save Physiotherapy data");
      }
    },
  });

  const deleteMutation = useMutation({
    mutationFn: deletePhysiotherapy,
    onSuccess: (data) => {
      const resp = data?.response;
      if (resp?.Apistatus === false) {
        toast.error(resp?.message || "Validation failed");
  ...
  useEffect(() => {
    if (
      physiotherapyData &&
      Array.isArray(physiotherapyData) &&
      physiotherapyData.length > 0
    ) {
      setRecords(physiotherapyData);
    }
  }, [physiotherapyData]);
  ...
  if (loadingMeta || loadingPhysiotherapy) {
    return (
    ...
    <div className="min-h-screen bg-muted">
      ...
      <main className="container mx-auto px-6 py-8 max-w-7xl">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-foreground">Physiotherapy</h1>
          <Button
            type="button"
