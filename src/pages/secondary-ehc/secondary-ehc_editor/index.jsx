...
export default function SecondaryEhc() {
  ...
  const {
    ...
    queryKey: ["secondaryEhcMeta"],
    queryFn: getSecondaryEhcMeta,
    staleTime: 5 * 60 * 1000,
    retry: 2,
  });
  const metadata = apiResponse?.response || {};
  const { data: secondaryEhcData, isLoading: isLoadingSecondaryEhc } = useQuery(
    {
      queryKey: ["secondaryEhc", slug],
      queryFn: async () => {
        if (!slug) return null;

        try {
          const data = await fetchSecondaryEhcBySlug(slug);
          return data;
        } catch (error) {
          if (
  ...
  useEffect(() => {
    if (secondaryEhcData) {
      setFormData({
        ...
        status_id: secondaryEhcData.status_id || null,
        insurance_co: secondaryEhcData.insurance_co || "",
        ref_initail: secondaryEhcData.ref_initail || "",
        claim_form: secondaryEhcData.claim_form || "",
        address: {
          unit_number: secondaryEhcData.address?.unit_number || "",
          street_number: secondaryEhcData.address?.street_number || "",
          street_name: secondaryEhcData.address?.street_name || "",
          city: secondaryEhcData.address?.city || "",
          province: secondaryEhcData.address?.province || "",
          postal_code: secondaryEhcData.address?.postal_code || "",
          country: secondaryEhcData.address?.country || "",
        },
        telephone: secondaryEhcData.telephone || "",
        ext: secondaryEhcData.ext || "",
        fax: secondaryEhcData.fax || "",
        ref_first_name: secondaryEhcData.ref_first_name || "",
        ref_last_name: secondaryEhcData.ref_last_name || "",
        ref_pgroup_no: secondaryEhcData.ref_pgroup_no || "",
        ref_idcard_no: secondaryEhcData.ref_idcard_no || "",
        ref_patient_id: secondaryEhcData.ref_patient_id || "",
        ref_policyholder_same: secondaryEhcData.ref_policyholder_same || false,
        ref_first_name_1: secondaryEhcData.ref_first_name_1 || "",
        ref_last_name_1: secondaryEhcData.ref_last_name_1 || "",
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <div className="flex gap-4">
          <Button
            onClick={() => queryClient.invalidateQueries(["secondaryEhcMeta"])}
          >
            Retry
          </Button>
          <Button
    ...
    <div className="min-h-screen bg-muted">
      ...
      <div className="bg-card border-b px-6 py-4">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-foreground font-medium">Secondary EHC</span>
        </div>
      </div>
