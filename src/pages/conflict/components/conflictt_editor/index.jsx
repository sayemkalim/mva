...
export default function ConflictSearchPage() {
  ...
  const mutation = useMutation({
    mutationFn: (data) => {
      ...
      if (id) {
        return updateConflict(id, data);
      } else {
        if (!slug) {
          return Promise.reject(
            new Error("Slug is required for creating conflict search")
          );
        }
        return createConflict({ slug, ...data });
      }
    },
    onSuccess: () => {
      toast.success(
      ...
      <nav className="bg-white border-b px-6 py-4 text-sm text-gray-600">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-900 font-medium">
            {id ? "Edit Conflict Search" : "Conflict Search"}
          </span>
        </div>
      </nav>
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t">
            ...
            >
              {mutation.isLoading ? (
                ...
                "Update Conflict Search"
              ) : (
                "Save Conflict Search"
              )}
            </Button>
          </div>
        </form>
