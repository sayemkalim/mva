...
export default function NonEngagementPage() {
  ...
  const { data: nonEngagementResponse, isLoading: loadingNonEngagement } =
    useQuery({
      queryKey: ["nonEngagement", slug],
      queryFn: () => fetchNonEngagementBySlug(slug),
      enabled: !!slug,
    });

  const nonEngagementData =
    nonEngagementResponse?.response || nonEngagementResponse || [];

  const saveMutation = useMutation({
    mutationFn: createNonEngagement,
    onSuccess: () => {
      toast.success("Non-Engagement data saved successfully!");
      queryClient.invalidateQueries(["nonEngagement", slug]);
    },
    onError: () => {
      toast.error("Failed to save Non-Engagement data");
    },
  });

  const deleteMutation = useMutation({
    mutationFn: deleteNonEngagement,
    onSuccess: () => {
  ...
  useEffect(() => {
    if (
      nonEngagementData &&
      Array.isArray(nonEngagementData) &&
      nonEngagementData.length > 0
    ) {
      setRecords(nonEngagementData);
    }
  }, [nonEngagementData]);
  ...
  if (loadingMeta || loadingNonEngagement) {
    return (
    ...
    <div className="min-h-screen bg-muted">
      ...
      <nav className="bg-card border-b px-6 py-4 text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-foreground font-medium">Non-Engagement</span>
        </div>
      </nav>

      <main className="container mx-auto px-6 py-8 max-w-7xl">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-foreground">Non-Engagement</h1>
          <Button
            type="button"
