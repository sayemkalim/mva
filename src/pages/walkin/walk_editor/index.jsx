...
import { fetchWalkBySlug } from "../helpers/fetchWalkBySlug";
import { createWalk } from "../helpers/createWalk";
import { deleteWalk } from "../helpers/deleteWalk";
...
export default function WalkPage() {
  ...
  const { data: walkResponse, isLoading: loadingWalk } = useQuery({
    queryKey: ["walk", slug],
    queryFn: () => fetchWalkBySlug(slug),
    enabled: !!slug,
  });

  const walkData = walkResponse?.response || walkResponse || [];
  ...
  useEffect(() => {
    if (walkData && Array.isArray(walkData) && walkData.length > 0) {
      setRecords(walkData);
    }
  }, [walkData]);
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <DatePicker
                  ...
                  onChange={(val) =>
                    handleRecordChange(index, "from_date", val)
                  }
                />
                ...
                <DatePicker
                  ...
                  onChange={(val) => handleRecordChange(index, "to_date", val)}
                />
