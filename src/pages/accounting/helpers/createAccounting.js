...
export const createAccounting = async ({ slug, ...payload }) => {
  if (!slug) throw new Error("Slug is required for creating");

  const apiResponse = await apiService({
    endpoint: `${endpoints.createAccounting}/${slug}`,
    method: "POST",
    data: payload,
...
export const updateAccounting = async (id, data) => {
  try {
    if (!id) throw new Error("ID is required for updating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.updateAccounting}/update/${id}`,
      method: "POST",
      data,
      headers: {
