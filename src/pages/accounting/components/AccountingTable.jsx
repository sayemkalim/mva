...
import { fetchAccountingList } from "../helpers/fetchAccountingList";
import { deleteAccounting } from "../helpers/deleteAccounting";
...
const AccountingTable = ({ slug, setBlogsLength, params = {} }) => {
  ...
  const { mutate: deleteSectionMutation, isLoading: isDeleting } = useMutation({
    mutationFn: (id) => deleteAccounting(id),
    onSuccess: () => {
      toast.success("Accountinglist deleted successfully.");
      queryClient.invalidateQueries(["accountinglist", slug]);
      onCloseDialog();
    },
    onError: () => {
      toast.error("Failed to delete accountinglist.");
    },
  });
  ...
  const onNavigateToEdit = (section) => {
    ...
    navigate(`/dashboard/accounting/edit/${section.id}`);
  };
