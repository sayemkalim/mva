...
import {
  createInsuranceDoc,
  updateInsuranceDoc,
} from "../../helpers/createInsuranceDoc";

export default function InsuranceDocPage() {
  ...
  const mutation = useMutation({
    mutationFn: (data) => {
      ...
      if (id) {
        return updateInsuranceDoc(id, data);
      } else {
        if (!slug) {
        ...
        return createInsuranceDoc({ slug, ...data });
      }
    },
    onSuccess: () => {
      toast.success(
    ...
    onError: (err) => {
      ...
      toast.error(
        err.message ||
        (id
          ? "Failed to update Insurance Ownership"
          : "Failed to save Insurance Ownership")
      );
    },
  });
      ...
      <nav className="bg-card border-b px-6 py-4 text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <span className="text-foreground font-medium">
            {id ? "Edit Insurance Ownership" : "Insurance Ownership"}
          </span>
        </div>
      </nav>
