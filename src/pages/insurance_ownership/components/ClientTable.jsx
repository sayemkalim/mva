...
import { fetchInsuranceDocList } from "../helpers/fetchInsuranceDocList";
import { deleteInsuranceDoc } from "../helpers/deleteInsuranceDoc";
...
const InsuranceOwnerTable = ({ slug, setBlogsLength, params = {} }) => {
  ...
  const {
    ...
    queryKey: ["insurancedocList", slug, params.search],
    queryFn: () => fetchInsuranceDocList(slug, { search: params.search || "" }),
    enabled: !!slug,
  });
  ...
  const { mutate: deleteSectionMutation, isLoading: isDeleting } = useMutation({
    mutationFn: (id) => deleteInsuranceDoc(id),
    onSuccess: () => {
      toast.success("Insurancedoc deleted successfully.");
      queryClient.invalidateQueries(["insurancedocList", slug]);
      onCloseDialog();
    },
    onError: () => {
      toast.error("Failed to delete Insurancedoc.");
    },
  });
  ...
  const onNavigateToEdit = (section) => {
    ...
    navigate(`/dashboard/insurance-ownership/edit/${section.id}`);
  };
...
export default InsuranceOwnerTable;
