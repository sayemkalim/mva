...
export const createInsuranceDoc = async ({ slug, ...payload }) => {
  if (!slug) throw new Error("Slug is required for creating");

  const apiResponse = await apiService({
    endpoint: `${endpoints.createInsuranceDoc}/${slug}`,
    method: "POST",
    data: payload,
...
export const updateInsuranceDoc = async (id, data) => {
  try {
    if (!id) throw new Error("ID is required for updating Section");

    const apiResponse = await apiService({
      endpoint: `${endpoints.updateInsuranceDoc}/update/${id}`,
      method: "POST",
      data,
      headers: {
