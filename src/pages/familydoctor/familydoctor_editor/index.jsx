...
import { Loader2, ChevronRight, CalendarIcon, Plus, Trash2 } from "lucide-react";
...
export default function FamilyDoctorPage() {
  ...
  const { data: familyDoctorResponse, isLoading: loadingFamilyDoctor } = useQuery({
    queryKey: ["familyDoctor", slug],
    queryFn: () => fetchFamilyDoctorBySlug(slug),
    enabled: !!slug,
  });

  const familyDoctorData = familyDoctorResponse?.response || familyDoctorResponse || [];
  ...
  useEffect(() => {
    if (familyDoctorData && Array.isArray(familyDoctorData) && familyDoctorData.length > 0) {
      setRecords(familyDoctorData);
    }
  }, [familyDoctorData]);
  ...
  if (loadingMeta || loadingFamilyDoctor) {
    return (
    ...
    <div className="min-h-screen bg-muted">
      ...
      <nav className="bg-card border-b px-6 py-4 text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-foreground font-medium">Family Doctor</span>
        </div>
      </nav>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <FloatingInput
                  ...
                  onChange={(e) =>
                    handleRecordChange(index, "invoice_amount", e.target.value)
                  }
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <FloatingInput
                  ...
                  onChange={(e) =>
                    handleRecordChange(index, "cnr_submitted_to_ab", e.target.value)
                  }
                />
                ...
                <SearchableSelect
                  ...
                  onChange={(val) =>
                    handleRecordChange(index, "who_paid_the_invoice_payment_id", val)
                  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <DatePicker
                  ...
                  value={record.date}
                  onChange={(val) =>
                    handleRecordChange(index, "date", val)
                  }
                />
                ...
                <SearchableSelect
                  ...
                  value={record.via_id}
                  onChange={(val) =>
                    handleRecordChange(index, "via_id", val)
                  }
                ...
                <DatePicker
                  ...
                  onChange={(val) =>
                    handleRecordChange(index, "police_report_submitted_to_ab", val)
                  }
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <SearchableSelect
                  ...
                  onChange={(val) =>
                    handleRecordChange(index, "who_paid_the_invoice_payment_2_id", val)
                  }
                  placeholder="Select who paid"
                />
