...
import { fetchHospitalReportBySlug } from "../helpers/fetchHospitalReportBySlug";
import { createHospitalReport } from "../helpers/createHospitalReport";
import { deleteHospitalReport } from "../helpers/deleteHospitalReport";
...
export default function HospitalReportPage() {
  ...
  const { data: hospitalReportResponse, isLoading: loadingHospitalReport } =
    useQuery({
      queryKey: ["hospitalReport", slug],
      queryFn: () => fetchHospitalReportBySlug(slug),
      enabled: !!slug,
    });

  const hospitalReportData =
    hospitalReportResponse?.response || hospitalReportResponse || [];

  const saveMutation = useMutation({
    mutationFn: createHospitalReport,
    onSuccess: (data) => {
      ...
      toast.success(resp?.message || "Hospital Report data saved successfully!");
      queryClient.invalidateQueries(["hospitalReport", slug]);
    },
    onError: (error) => {
      ...
      if (errorData?.Apistatus === false) {
        ...
        toast.error(errorData?.message || "Failed to save Hospital Report data");
      }
    },
  });

  const deleteMutation = useMutation({
    mutationFn: deleteHospitalReport,
    onSuccess: (data) => {
      const resp = data?.response;
      if (resp?.Apistatus === false) {
        toast.error(resp?.message || "Validation failed");
  ...
  useEffect(() => {
    if (
      hospitalReportData &&
      Array.isArray(hospitalReportData) &&
      hospitalReportData.length > 0
    ) {
      setRecords(hospitalReportData);
    }
  }, [hospitalReportData]);
  ...
  if (loadingMeta || loadingHospitalReport) {
    return (
