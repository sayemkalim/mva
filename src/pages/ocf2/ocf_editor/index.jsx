...
import { createOcf2 } from "../helpers/createOcf2";
...
export default function Ocf2Page() {
  ...
  const { data: ocf2Data, isLoading: loadingOcf2 } = useQuery({
    queryKey: ["ocf2", slug],
    queryFn: () => fetchOcf2BySlug(slug),
    enabled: !!slug,
  });

  const saveMutation = useMutation({
    mutationFn: createOcf2,
    onSuccess: () => {
      toast.success("OCF-2 data saved successfully!");
    },
    onError: () => {
      toast.error("Failed to save OCF-2 data");
    },
  });
  ...
  useEffect(() => {
    if (ocf2Data) {
      setFormData({
        completed_by_id: ocf2Data.completed_by_id || "",
        date_of_completion: ocf2Data.date_of_completion || "",
        date: ocf2Data.date || "",
        via_id: ocf2Data.via_id || "",
        date_2: ocf2Data.date_2 || "",
        via_2_id: ocf2Data.via_2_id || "",
      });
    }
  }, [ocf2Data]);
    ...
    <div className="min-h-screen bg-muted">
      ...
      <nav className="bg-card border-b px-6 py-4 text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-foreground font-medium">OCF-2</span>
        </div>
      </nav>

      <main className="container mx-auto px-6 py-8 max-w-7xl">
        <h1 className="text-2xl font-bold mb-6 text-foreground">OCF-2 Form</h1>
        <form
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <SearchableSelect
              label="Completed By"
              options={meta.tracking_ocf_2_completed_by || []}
